...
<body>
  <div class="relative flex h-auto min-h-screen w-full flex-col bg-white group/design-root overflow-x-hidden"
    ...
    <div class="layout-container flex h-full grow flex-col">
      <header
        class="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#f4f3f0] px-10 py-3">
      ...
      <div id="app" class="gap-1 px-6 flex flex-1 justify-center py-5">
        ...
        <div class="layout-content-container flex flex-col max-w-[920px] flex-1">
          <div v-if="!extras" class="flex flex-wrap justify-between gap-3 p-4">
            <p class="text-[#181511] tracking-light text-[32px] font-bold leading-tight min-w-72">New Order</p>
          </div>
          <div v-if="!extras" class="pb-3">

            <div class="flex border-b border-[#e6e1db] px-4 gap-8">
              <a v-for="categoria in categorias" @click="categoria_seleccionada = categoria.id"
                :class="{'border-b-[3px] border-b-[#181511]': categoria_seleccionada === categoria.id}"
          ...
          <h3 v-if="!extras" class="text-[#181511] text-lg font-bold leading-tight tracking-[-0.015em] px-4 pb-2 pt-4">
            @{{nomcategoria}}</h3>
            ...
            <div class="flex items-stretch justify-between gap-4 rounded-lg">
              <div class="flex flex-[2_2_0px] flex-col gap-4">
                <div class="flex flex-col gap-1">
                  ...
                  <p class="text-[#181511] text-base font-bold leading-tight">@{{producto.categoria}} -
                    $@{{producto.precio}}</p>
        ...
        <div v-if="!extras" class="layout-content-container flex flex-col w-[360px]">
          <h2 class="text-[#181511] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Order
            Summary</h2>
          <div v-for="(item, index) in orden"
            ...
            <div class="flex flex-col">
              ...
              <div>
                <select v-model="item.tamanio"
                  class="w-full mt-1 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-400">
                  <option v-for="tamanio in tamanios" :value="tamanio.id">@{{tamanio.nombre}}</option>
                </select>
              </div>
            </div>
            <div>
              <button type="button" @click="activar_extra(index)">Extras</button>
            </div>
            <div>
              <button @click="eliminar_orden(index)"
                class="flex items-center justify-center w-9 h-9 rounded-md bg-white border border-[#e6e1db] hover:bg-[#fff7f0]"
                aria-label="Eliminar item"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
          ...
          <h2 class="text-[#181511] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Loyalty
            Program</h2>
          ...
          <div class="flex items-center gap-4 bg-white px-4 min-h-14 justify-between">
            ...
            <div class="shrink-0">
              <p class="text-[#181511] text-base font-normal leading-normal">$@{{ order_summary.subtotal }}</p>
            </div>
          </div>
          <div class="flex items-center gap-4 bg-white px-4 min-h-14 justify-between">
            <p class="text-[#181511] text-base font-normal leading-normal flex-1 truncate">Tax (@{{ impuesto*100 }}%)
            </p>
            <div class="shrink-0">
              <p class="text-[#181511] text-base font-normal leading-normal">$@{{ order_summary.impuesto }}</p>
            </div>
          </div>
          <div class="flex items-center gap-4 bg-white px-4 min-h-14 justify-between">
            <p class="text-[#181511] text-base font-normal leading-normal flex-1 truncate">Total</p>
            <div class="shrink-0">
              <p class="text-[#181511] text-base font-normal leading-normal">$@{{ order_summary.total }}</p>
            </div>
          </div>
          <h2 class="text-[#181511] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Payment
            Method</h2>
        ...
        <div v-if="extras" class="px-100 flex flex-1 justify-center py-5">
          <div class="layout-content-container flex flex-col w-[512px] max-w-[512px] py-5 max-w-[960px] flex-1">
            <div class="flex flex-wrap justify-between gap-3 p-4">
              <p class="text-[#181511] tracking-light text-[32px] font-bold leading-tight min-w-72">Customize Your Drink
              </p>
            ...
            <div>
              <div v-for="cextra in categoria_complementos">
                <h3 class="text-[#181511] text-lg font-bold leading-tight tracking-[-0.015em] px-4 pb-2 pt-4">@{{
                  cextra.nombre }}</h3>
                <div class="flex flex-wrap gap-3 p-4">
                  <label v-for="extra in dame_extras(cextra.id)" @click="agregar_extra(extra)"
                    class="text-sm font-medium leading-normal flex items-center justify-center rounded-lg border border-[#e6e1db] px-4 h-11 text-[#181511] has-[:checked]:border-[3px] has-[:checked]:px-3.5 has-[:checked]:border-[#ec9213] relative cursor-pointer">
              ...
              <p class="text-[#897961] text-sm font-normal leading-normal pb-3 pt-1 px-4">Total Customization Cost:
                $1.30</p>
              <div class="flex px-4 py-3 justify-end">

                <button @click="extras=false">
                  Cancelar
                </button>
                <button
                  class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#ec9213] text-[#181511] text-sm font-bold leading-normal tracking-[0.015em]">
        ...
        <!--Contenido para agregar extras a la bebida-->
      ...
      <script>
        var app = new Vue({
          ...
          data: function () {
            return {
              productos: []
              , categorias: <?php echo json_encode($categorias);?> //sincrona
              , complementos: <?php echo json_encode($extras);?>
          ...
          , computed: {
            ...
            // ,impuesto: function(){
            //     return this.subtotal * parseFloat(this.impuesto || 0);
            // }
            , total: function () {
              return 0;
            }
            ...
            , order_summary: function () {
              ...
              ty.impuesto = ty.subtotal * parseFloat(this.impuesto || 0);
          ...
          , methods: {
            ...
            , guardar_orden: function () {
              ...
              //}
              //}
              xhr.send(JSON.stringify({ orden: this.orden }));
            }
